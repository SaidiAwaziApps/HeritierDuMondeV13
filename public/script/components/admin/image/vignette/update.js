 ...
 * CRÉATION D'UN ITEM
 ******************************************/
function createVgnItem(vignette = null) {
    const vgnItem = document.createElement('div');
    vgnItem.classList.add('card', 'vgn-img-item');

    if(vignette) {
        vgnItem.setAttribute('id',''+vignette.id+'');
    }

    const iframeHTML = vignette?.iframe ?? '';

    vgnItem.innerHTML = `
        <div class="card-body">
            <div class="vgn-img-bloc">
                <div class="vgn-iframe-input" style="${iframeHTML ? 'display:none' : 'display:flex'}">
                    <span contenteditable="false">${DEFAULT_IFRAME_TEXT}</span>
                </div>

                <div class="vgn-iframe-result" style="${iframeHTML ? 'display:flex' : 'display:none'}">
                    ${iframeHTML}
                </div>
            </div>
...
/*******************************************************
 * CREATION D' UN ELEMENT INPUT IFRAMES
 * *****************************************************/
...
/*******************************************************
 * CREATION D' UN ELEMENT INPUT REMOVE_VGN_INPUT
 * *****************************************************/
function createVgnRemoveInput(id) {
    const input = document.createElement('input');
    input.setAttribute('type','hidden');
    input.setAttribute('name','remove_vgns_id[]');
    input.setAttribute('id','remove_vgns_id');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // *** IMAGES PLATFORM (VIGNETTES) *** 
    const platformImages = images.filter(item => item.img_source.toLowerCase() == 'vignette');
    ...
    if(platformImages && platformImages.length > 0) {
        ...
        for (let i = 0; i < 3; i++) {
            vgnImgsContent.appendChild(createVgnItem(null));
    ...
    vgnImgsContent.addEventListener('click', function (e) {
        ...
        if (closeBtn) {
            ...
            const inputToRemove = (item,inputIndex,value) => {
                if(item.getAttribute('id')) { // Presence de l' attribut (donnee existant depuis le backend)
                    vgnInputs.appendChild(createVgnRemoveInput(item.getAttribute('id')));  
                    platformImages.splice(index,1); //Supprime l' element du tableau vignettes
                } else {
                    value ? vgnInputs.children[inputIndex].remove() 
                          : vgnInputs.children[inputIndex].value = null;  // Supprime input element 
                }
            }
            ...
            if (allItems.length > 3) {
                inputToRemove(allItems[index],inputIndex(),index); 
                allItems[index].remove();
            } else {
                inputToRemove(allItems[index],inputIndex(),null);
    ...
    /* ******************************************
     * GESTION DU BLUR (SORTIE ZONE EDITION)
     * *****************************************/
    vgnImgsContent.addEventListener('blur', function (e) {
        ...
        if (isValidIframe(rawHTML)) {
            ...
            vgnInputs.children[inputIndex()].value = rawHTML;
