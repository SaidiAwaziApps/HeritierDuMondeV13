...
/*  ******************************************************************
 *  INITIALE LE CONTENU DU FORMULAIRE
 *  ******************************************************************/
function init() {
    /* ---- Contenu par defaut ---- */
    document.querySelector('textarea[id="contenu"]').value = article.contenu;

    /* ---- En cas de la presence d'image entente ---- */
    if(article.header_image) {
        // Element label
        const labelElement = document.querySelector('label[for="header_image"]');
        // S' agissant d' une video
        if(isVideo(article.header_image)) {
            var content = '<video controls class="rounded-thumnail cover" style="width: 100%;height: 100%;">'+
                '<source src="'+window.STORAGE_PATH_URL+'/'+article.header_image+'"/>'
            +'<video>';
            labelElement.innerHTML = ''+content+'';
        } else {
            labelElement.innerHTML = '<span id="header-image-alternative">'+
                                        '<i class="fa fa-plus"></i> Image entente'
                                    +'<span>';
            labelElement.style.backgroundImage = 'url('+window.STORAGE_PATH_URL+'/'+article.header_image+')';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /* ---- Appel a la methode initialisation (init) ---- */
    init(); // init()
});
